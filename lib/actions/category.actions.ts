...
export async function updateCategory(categoryId: string, input: unknown): Promise<ActionResult> {
  ...
  try {
    ...
    await prisma.category.update({
      where: { id: categoryId },
      data: { name: parsed.data.name, iconUrl: parsed.data.iconUrl || null },
    });
...
/** Sube o baja una categoría un puesto — intercambia su `order` con el vecino. */
export async function reorderCategory(categoryId: string, direction: "up" | "down"): Promise<ActionResult> {
  ...
  if (swapWith < 0 || swapWith >= all.length) return { ok: true, data: undefined };
