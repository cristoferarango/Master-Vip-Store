...
/** Datos para el checkout: precio, tipo de producto y el Yape propio del proveedor. */
...
export async function createPurchaseRequest(
  ...
  try {
    ...
    const { requestId, providerUserId, productName, amount } = await prisma.$transaction(async (tx) => {
      ...
      if (product.type === "ACTIVACION" && !parsed.data.clientEmail) {
      ...
      const request = await tx.purchaseRequest.create({
        data: {
          ...
          clientEmail: product.type === "ACTIVACION" ? parsed.data.clientEmail : undefined,
        },
      });
