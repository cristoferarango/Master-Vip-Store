...
export async function POST(request: Request) {
  ...
  // y uno más ajustado por IP+usuario (evita fuerza bruta dirigida a una cuenta).
  ...
  if (!checkRateLimit(`login:acct:${ip}:${parsed.data.username.toLowerCase()}`, 8, 10 * 60 * 1000)) {
    return NextResponse.json(
      { ok: false, error: "Demasiados intentos con este usuario. Espera unos minutos y vuelve a intentar." },
      { status: 429 }
    );
  }
