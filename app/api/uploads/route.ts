...
const ALLOWED_FOLDERS: UploadFolder[] = ["productos", "avatares", "yape-qr", "pagos", "iconos"];
...
// SVG solo se permite en "iconos" (lo sube el admin, no el público) y se
// rechaza si trae <script>/on-eventos — mitiga XSS al servirlo directo.
function isSvgSafe(text: string): boolean {
  const lower = text.toLowerCase();
  return !/<script|on\w+\s*=|javascript:/.test(lower);
}
...
export async function POST(request: Request) {
  ...
  if (!extension || (file.type === "image/svg+xml" && folder !== "iconos")) {
  ...
  if (file.type === "image/svg+xml") {
    ...
    if (!isSvgSafe(text)) {
      return NextResponse.json({ ok: false, error: "SVG no permitido (contiene código embebido)" }, { status: 400 });
    }
  } else {
