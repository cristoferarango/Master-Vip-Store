      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
        {categories.map((c) => (
          <Card key={c.id} className="flex flex-col items-center gap-2 py-6 text-center">
            <span className="animate-float relative flex h-12 w-12 items-center justify-center overflow-hidden rounded-2xl bg-primary/15 text-accent">
              {c.iconUrl ? <Image src={c.iconUrl} alt={c.name} fill className="object-contain p-2" unoptimized /> : <Library size={22} />}
            </span>
