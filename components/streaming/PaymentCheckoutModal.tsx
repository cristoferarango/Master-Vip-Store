...
export function PaymentCheckoutModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const isActivation = info?.productType === "ACTIVACION" || info?.productType === "ACTIVACION2";
    if (isActivation && !clientEmail) {
      setError("Ingresa el correo donde se activará el servicio.");
      return;
    }
    if (info?.productType === "ACTIVACION2" && info.activacion2RequestsPassword && !clientPassword) {
      setError("Ingresa la contraseña de tu cuenta.");
      return;
    }
    ...
    const result = await createPurchaseRequest({
      ...
      clientEmail: isActivation ? clientEmail : undefined,
      clientPassword: info?.productType === "ACTIVACION2" ? clientPassword || undefined : undefined,
    });
    setSubmitting(false);
    ...
    setRequestTime(new Date().toLocaleTimeString("es-PE", { hour: "2-digit", minute: "2-digit" }));
    setDone(true);
    router.refresh();
  }
    ...
    <Modal open={open} onClose={handleClose} title={done ? "Solicitud enviada" : "Pagar con Yape"}>
      ...
      {info && !done && (
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="rounded-xl border border-border bg-surface p-4 text-center">
            ...
            {info.yapeQrUrl ? (
              <div className="relative mx-auto mb-3 h-28 w-28 overflow-hidden rounded-lg border border-border bg-white">
                <Image src={info.yapeQrUrl} alt={`QR de Yape de ${info.providerName}`} fill className="object-contain p-1.5" unoptimized />
              </div>
            ) : (
          ...
          {(info.productType === "ACTIVACION" || info.productType === "ACTIVACION2") && (
            <Input
          ...
          {info.productType === "ACTIVACION2" && info.activacion2RequestsPassword && (
            <Input
              label="Contraseña de esa cuenta"
              type="password"
              required
