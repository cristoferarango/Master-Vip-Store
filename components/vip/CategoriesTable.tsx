...
import { Library, Plus, ArrowUp, ArrowDown } from "lucide-react";
import { Button } from "@/components/ui/Button";
import { deleteCategory, reorderCategory } from "@/lib/actions/category.actions";
...
export function CategoriesTable({ categories }: { categories: CategoryRow[] }) {
  ...
  function openEdit(c: CategoryRow) {
    setEditTarget({ id: c.id, name: c.name, iconUrl: c.iconUrl });
    setModalOpen(true);
  }
          ...
          {categories.map((c, i) => (
            <div key={c.id} className="flex items-center justify-between gap-3 px-4 py-3">
              <div className="flex items-center gap-3">
                <span className="relative flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-primary/15 text-accent">
                  {c.iconUrl ? (
                    <Image src={c.iconUrl} alt={c.name} fill className="object-contain p-1" unoptimized />
                  ) : (
                    <Library size={18} />
                  )}
                </span>
              ...
              <div className="flex shrink-0 items-center gap-1.5">
                <Button size="sm" variant="outline" disabled={i === 0} onClick={() => handleMove(c.id, "up")} aria-label="Subir">
                  <ArrowUp size={14} />
                </Button>
                <Button size="sm" variant="outline" disabled={i === categories.length - 1} onClick={() => handleMove(c.id, "down")} aria-label="Bajar">
                  <ArrowDown size={14} />
                </Button>
