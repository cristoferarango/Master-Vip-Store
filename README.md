...
### 🏪 Proveedores (`/provee`) — panel de vendedor
...
### 👑 Master / VIP (`/owner`) — panel del dueño
...
Cada **proveedor** define su propio horario (`Abre` / `Cierra`) desde su Perfil. El dueño puede definir además un horario **general de la plataforma** desde el Panel Master. Un producto solo es comprable si **ambos** horarios (plataforma y proveedor) están abiertos en ese momento (hora de Lima); si no, el botón muestra "Fuera de horario" en vez de "Comprar ahora". Dejar los campos vacíos = disponible 24 h. Se valida tanto en la interfaz como en el servidor al crear la solicitud de compra.
...
```
app/(main)/       páginas de la tienda (Streaming) — grupo de rutas, sin prefijo en la URL (es el dominio raíz "/")
app/provee/       páginas del panel proveedor (rutas bajo /provee)
app/owner/        páginas del panel dueño / Master (rutas bajo /owner)
